...
const WithdrawalReceipt = ({close}) => {

  let amount = localStorage.getItem("withdrawalamount")
  let account = localStorage.getItem("receivingaccount")
  let beneficiary = localStorage.getItem("beneficiary")
  let remainder = localStorage.getItem("remainder")

  const id = useParams().id

  const handleconfirm = () => {
  window.location.replace(`/dashboard/withdraw/enterpin/${id}`)
  }
  ...
  return (
<div className="flex px-2 pt-4 sm:pt-12">
...
<div className="relative overflow-hidden min-h-[8rem] bg-gray-900 text-center rounded-t-xl">
  {/* <!-- Close Button --> */}
  <div onClick={close} className="absolute top-2 right-2">
    <button type="button" className="inline-flex flex-shrink-0 justify-center items-center h-8 w-8 rounded-md text-gray-500 hover:text-gray-400 focus:outline-none focus:ring-2 focus:ring-gray-400 focus:ring-offset-2 focus:ring-offset-white transition-all text-sm  " data-hs-overlay="#hs-bg-gray-on-hover-cards">
...
<div className="p-4 sm:p-7 overflow-y-auto">
    ...
    <ul className="mt-3 flex flex-col">
      ...
      <li className="inline-flex items-center gap-x-2 py-3 px-4 text-sm font-semibold bg-gray-50 border text-gray-800 -mt-px first:rounded-t-lg first:mt-0 last:rounded-b-lg  ">
        <div className="flex items-center justify-between w-full">
          ...
          <span>{formatter.format(remainder)}</span>
        </div>
      </li>
    </ul>
  </div>
  ...
  {/* <div className="mt-5 flex justify-end gap-x-2">
   ...
   <div className="mt-4">
   {/* <NavLink to={`/dashboard/withdraw/withdrawing/${id}`} className=" flex  justify-center  w-full bg-blue-300 rounded-3xl px-4 py-2 text-xl md:text-xl tracking-wide font-semibold disabled:bg-black/70">
      Confirm
   </NavLink> */}

   <a onClick={() => handleconfirm()} className=" flex  justify-center  w-full bg-blue-300 rounded-3xl px-4 py-2 text-xl md:text-xl tracking-wide font-semibold disabled:bg-black/70 cursor-pointer">
      Confirm
   </a>
   </div>
  {/* <!-- End Buttons --> */}
  ...
  <div className="mt-5 sm:mt-10">
    <p className="text-sm text-gray-500">If you have any questions, please contact us at <a className="inline-flex items-center gap-x-1.5 text-blue-600 decoration-2 hover:underline font-medium" href="#">info@savely.com</a> or call at <a className="inline-flex items-center gap-x-1.5 text-blue-600 decoration-2 hover:underline font-medium" href="tel:+1898345492">+1 898-34-5492</a></p>
  </div>
</div>
{/* <!-- End Body --> */}
</div>
